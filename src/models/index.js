...
export const createForm = () => database.ref().child('forms').push().key;

export const editFormById = (id, name, fields) => {
    return new Promise((resolve, reject) => {
        database.ref('/forms/' + id).set({name, fields}).then(error => {
...
export const removeFormById = (id) => {
    return new Promise((resolve, reject) => {
        database.ref('/forms/' + id).remove().then(error => {
            if (error) reject(error);
            else resolve("Success");
        })
    });
...
export const getFormById = (id) => {
    return new Promise((resolve, reject) => {
        database.ref('/forms/' + id).once('value').then(snapshot => {
