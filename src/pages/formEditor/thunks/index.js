...
export const createForm = (createId) => {
    ...
    debugger;
        ...
        editFormById(createId, name, fields).then(() => {
            const areFormsLoaded = getAreFormsLoaded(getState());

            dispatch(addField(INITIAL_FIELD_DATA));
            dispatch(setIsLoaded(true));
            if (areFormsLoaded)
                addForm({id: createId, name, fields: 1});
            history.pushState(null, null, window.location.href.replace('new', `e/${createId}`));
        }).then(() => showInfoMessage("Form Created")).catch(showErrorMessage)
    }
};
